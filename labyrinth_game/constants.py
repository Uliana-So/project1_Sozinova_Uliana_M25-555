ROOMS = {
    ...
    'crypt': {
        'description': 'Подземная крипта. Здесь очень тихо... слишком тихо. В углу виден блеск старого амулета.',
        'exits': {'east': 'hall', 'north': 'mirror_room'},
        'items': ['bone_amulet'],
        'puzzle': ('На стене: "Я всегда с тобой, но ты не можешь меня увидеть. Кто я?" (одно слово)', 'тень')
    },
    'illusion_corridor': {
        'description': 'Коридор иллюзий. Пол кажется бесконечным. Тени двигаются сами по себе.',
        'exits': {'south': 'echo_chamber', 'north': 'time_room', 'west': 'hall'},
        'items': ['mirror_shard'],
        'puzzle': (
            'Стены спрашивают: "Во что невозможно смотреть прямо, но оно всегда рядом?" (одно слово)',
            'будущее'
        )
    },
    'time_room': {
        'description': 'Комната времени. В центре — песочные часы, в них застыл песок.',
        'exits': {'south': 'illusion_corridor', 'east': 'music_hall'},
        'items': ['time_dust'],
        'puzzle': (
            'На пьедестале выгравировано: "Назови, что всегда движется вперёд, но остаётся позади?"',
            'время'
        )
    },
    'memory_vault': {
        'description': 'Хранилище памяти. Люди забывают, а это место помнит всё.',
        'exits': {'east': 'treasure_room'},
        'items': ['forgotten_key'],
        'puzzle': (
            'Шёпот: "Что можно потерять, не заметив? Что вернуть сложно, а забыть легко?"',
            'память'
        )
    },
}
